<template>
  <div>
    ...
    <el-row class="info" :gutter="0">
      ...
      <el-col :span="7">
        <el-card shadow="hover">
          <div class="item-group-d-graph" ref="itemDistributionChart"></div>
        </el-card>
      </el-col>
      <el-col :span="7">
        <el-card shadow="hover">
      ...
      .then((res) => {
        ...
        if (code === 20000) {
          ...
          const itemGroupOption = {
            ...
            xAxis: {
              type: "category",
              data: data.itemGroupData.map((item) => item.group),
              axisLine: {
                lineStyle: {
                  color: "#ff2510",
                },
            ...
            series: [
              {
                name: "分组数量",
                data: data.itemGroupData.map((item) => {
                  return {
                    value: item.number,
                    itemStyle: {
                      color: item.color,
          ...
          const itemDistributionOption = {
            ...
            color: data.itemGroupData.map((item) => item.color),
            series: [
              {
                data: data.itemGroupData.map((item) => {
                  return {
          ...
          const itemStockOption = {
            ...
            series: [
              {
                name: "入库",
                data: data.itemStockData.in,
                type: "line",
              },
              {
                name: "出库",
                data: data.itemStockData.out,
                type: "line",
              },
            ],
          };
